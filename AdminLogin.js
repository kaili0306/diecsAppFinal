...
export default class LoginScreen extends Component{
  ...
  async _readSetting(){
    ...
    try{
      ...
      if(id !== null){
        this.props.navigation.navigate('adminDrawer',{id:id});
      }
    }catch(error){
  ...
  login = () => {
    ...
    if (id == "") {
      ...
      fetch(config.settings.serverPath + '/php_rest_diecs/login/loginAdmin.php', {
        method: 'post',
        header: {
    ...
    render(){
        return(
          <KeyboardAvoidingView >
        <ImageBackground style={imageStyle.utarImage} source={require('./Images/utar2.jpg')}>
        <View style={{ flex: 1,justifyContent: 'center', backgroundColor: 'rgba(255,255,255,0.4)' }}>
            ...
            <View style={{flexDirection:'row'}}>
                <TouchableOpacity style={imageStyle.tabStyle1} value="student" onPress={() => this.props.navigation.navigate('StudentLogin')}>
                  <Text style={{ fontSize: 17, fontWeight: 'bold', alignSelf: 'center' }}>STUDENT</Text>
                </TouchableOpacity>

                <TouchableOpacity style={imageStyle.tabStyle} value="admin" onPress={() => this.props.navigation.navigate('AdminLogin')}>
